        ...
        duplicatedConnection.cluster("slots", utils_2.timeout((err, result) => {
          ...
          for (let i = 0; i < result.length; ++i) {
            ...
            for (let j2 = 2; j2 < items.length; j2++) {
              ...
              nodes.push(items[j2]);
              keys2.push(items[j2].host + ":" + items[j2].port);
            }
...
var import_redis_commands = __toESM(require_redis_commands());
...
var import_fengari2 = __toESM(require("fengari"));
var import_fengari_interop2 = __toESM(require("fengari-interop"));

// src/command.js
var import_lodash = __toESM(require("lodash"));
var import_standard_as_callback = __toESM(require("standard-as-callback"));
var import_ioredis = __toESM(require_built());
...
function processReply(result, commandName) {
  if (Command.transformers.reply[commandName]) {
    ...
    if (commandName === "hgetall") {
      newResult = import_lodash.default.flatten(Object.entries(result));
    }
...
// src/lua.js
var import_fengari = __toESM(require("fengari"));
var import_fengari_interop = __toESM(require("fengari-interop"));
var {
  lua,
  lualib,
  lauxlib,
...
function subscribeToChannel(instance, chan, channels, isPattern) {
  if (!channels.instanceListeners.has(chan)) {
    channels.instanceListeners.set(chan, /* @__PURE__ */ new Map());
  }
...
function psubscribe(...args) {
  args.forEach((pattern2) => {
    if (!this.patternChannels.instanceListeners) {
      this.patternChannels.instanceListeners = /* @__PURE__ */ new Map();
    }
...
function sadd(key, ...vals) {
  if (!this.data.has(key)) {
    this.data.set(key, /* @__PURE__ */ new Set());
  }
  let added = 0;
...
function sdiff(ours, ...theirs) {
  ...
  const ourSet = this.data.has(ours) ? this.data.get(ours) : /* @__PURE__ */ new Set();
  const theirSets = theirs.map((key) => this.data.has(key) ? this.data.get(key) : /* @__PURE__ */ new Set());
...
function sinter(...keys2) {
  ...
  const sets = keys2.map((key) => this.data.has(key) ? this.data.get(key) : /* @__PURE__ */ new Set());
...
function smove(source, destination, member) {
  ...
  if (!this.data.has(destination)) {
    this.data.set(destination, /* @__PURE__ */ new Set());
  }
...
function spop(key, count) {
  ...
  const set2 = this.data.get(key) || /* @__PURE__ */ new Set();
...
function subscribe(...args) {
  args.forEach((chan) => {
    if (!this.channels.instanceListeners) {
      this.channels.instanceListeners = /* @__PURE__ */ new Map();
    }
...
function hscanStream(key, opt = {}) {
  ...
  options.key = key;
  return new ReadableScan(this.hscan, options);
}
...
function sscanStream(key, opt = {}) {
  const options = opt instanceof Object ? opt : {};
  options.key = key;
  return new ReadableScan(this.sscan, options);
}
...
function zscanStream(key, opt = {}) {
  const options = opt instanceof Object ? opt : {};
  options.key = key;
  return new ReadableScan(this.zscan, options);
}
